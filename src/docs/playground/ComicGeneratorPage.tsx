...
export function ComicGeneratorPage() {
  ...
  const cycleBackground = useCallback(() => {
    ...
    patch({
      background:
        values[(values.indexOf(state.background) + 1) % values.length],
    });
  ...
  const dropSection = (event: DragEvent<HTMLDivElement>) => {
    ...
    if (SECTION_TYPES.includes(type as SectionType))
      handleAddSection(type as SectionType);
  };
  ...
  useEffect(() => {
    const handleShortcut = (event: KeyboardEvent) => {
      ...
      if (
        ...
        event.preventDefault();
        setDevice(
          ({ "1": "desktop", "2": "tablet", "3": "mobile" } as const)[
            event.key as "1" | "2" | "3"
          ],
        );
      }
  ...
  return (
    <article className="cg-app">
      <a href="#cg-canvas" className="cg-skip-link">
        Skip to canvas
      </a>
      <header className="cg-app-header">
        ...
        <div className="cg-header-status" aria-live="polite">
          <i /> Live ·{" "}
          {state.sections.filter((section) => !section.hidden).length}/
          {state.sections.length} visible
        </div>
        <div
          className="cg-mobile-tabs"
          role="group"
          aria-label="Generator panels"
        >
        ...
        <Button
          ...
          variant="danger"
          effect="pop"
          className="cg-random-button"
          onClick={randomize}
          loading={isRandomizing}
          disabled={isRandomizing}
        >
          {!isRandomizing ? <Dice5 aria-hidden="true" /> : null}
          {isRandomizing ? "Shuffling Panels…" : "Random Comic"}
        </Button>
      </header>

      <div
        className={`cg-workspace cg-pane-${mobilePane} ${codeOpen ? "has-code" : ""}`}
      >
        <SectionLibrary
        ...
        <main className="cg-canvas-area" id="cg-canvas" tabIndex={-1}>
          <CanvasToolbar
            ...
            zoom={zoom}
            selectedLabel={
              selected ? SECTION_MAP[selected.type].label : undefined
            }
          ...
          >
            <div className="cg-drop-overlay" aria-hidden="true">
              <Sparkles />
              <strong>Drop to add section</strong>
              <span>It will appear at the end of the page</span>
            </div>
            <div
        ...
        <div className={`cg-bottom-dock ${codeOpen ? "is-open" : ""}`}>
          <button
            type="button"
            className="cg-dock-toggle"
            onClick={() => setCodeOpen((open) => !open)}
            aria-expanded={codeOpen}
            aria-controls="cg-code-panel"
          >
            <span>04 · Code & export</span>
            <small>{state.sections.length} sections · synced</small>
            {codeOpen ? <ChevronDown /> : <ChevronUp />}
          </button>
          <div id="cg-code-panel">
            {codeOpen ? <CodePanel state={state} /> : null}
          </div>
        </div>
      </div>
      ...
      <Dialog open={resetOpen} onOpenChange={setResetOpen}>
        <DialogContent
          ...
          size="sm"
          showClose
          className="cg-reset-dialog"
        >
          <DialogHeader>
            <DialogTitle>Reset this comic?</DialogTitle>
            <DialogDescription>
              This replaces the current composition and visual settings with the
              default seven-section page.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
